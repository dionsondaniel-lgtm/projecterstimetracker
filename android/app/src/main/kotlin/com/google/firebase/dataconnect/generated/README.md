...
with service ID `time-tracker-app`
hosted in `us-east4`.
...
is the query named "ListProjectsForUser",
which can be accessed via the [ExampleConnector.listProjectsForUser] property.


An example of the property for a mutation
is the mutation named "CreateTimeEntry",
which can be accessed via the [ExampleConnector.createTimeEntry] property.
...
For example, the "ListProjectsForUser" query has no variables
and can be executed via the
[ExampleConnector.listProjectsForUser]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listProjectsForUser.execute()
println("ListProjectsForUser query returned: ${queryResult.data}")
```
...
For example, the "CreateTimeEntry" mutation has 4 required variables ("projectId", "taskId", "startTime", and "endTime")
and can be executed via the [ExampleConnector.createTimeEntry]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createTimeEntry.execute(projectId=java.util.UUID.randomUUID(), taskId=java.util.UUID.randomUUID(), startTime=Timestamp(1678416110, 211021812), endTime=Timestamp(1074791461, 869335796))
println("CreateTimeEntry mutation returned: ${mutationResult.data}")
```
...
For example, the "UpdateProject" mutation has 4 optional variables ("name", "description", "billingRatePerHour", and "status")
and can be executed via the [ExampleConnector.updateProject]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.updateProject.execute(id=java.util.UUID.randomUUID()) {
  name = "waldo"
  description = "baz"
  billingRatePerHour = 3474.16
  status = "garply"
}
println("UpdateProject mutation returned: ${mutationResult.data}")
```
